  ...
  useEffect(() => {
    ...
    class Explosion {
      ...
      draw() {
        ...
        this.particles.forEach(p => {
          const finalOpacity = Math.max(0, this.life);
    ...
    class Comet {
      ...
      reset() {
        ...
        if (side === 0) { this.x = -50; this.y = Math.random() * height; this.vx = 2.5; this.vy = (Math.random()-0.5)*2; }
        else if (side === 1) { this.x = width+50; this.y = Math.random() * height; this.vx = -2.5; this.vy = (Math.random()-0.5)*2; }
        else { this.x = Math.random()*width; this.y = -50; this.vx = (Math.random()-0.5)*2; this.vy = 2.5; }
        this.life = 1.0; this.active = true;
      }
      update() {
        this.x += this.vx; this.y += this.vy; this.life -= 0.004;
        if (this.life <= 0) this.active = false;
      }
      draw() {
        ctx.save();
        const g = ctx.createLinearGradient(this.x, this.y, this.x-this.vx*25, this.y-this.vy*25);
        g.addColorStop(0, `rgba(255,255,255,${this.life * 0.8})`); 
        g.addColorStop(1, "rgba(255,255,255,0)");
        ctx.strokeStyle = g; ctx.lineWidth = 1.5; ctx.beginPath();
        ctx.moveTo(this.x, this.y); ctx.lineTo(this.x-this.vx*18, this.y-this.vy*18); ctx.stroke();
        ctx.restore();
      }
    }
    ...
    function handleInteraction(ex, ey) {
      ...
      if (closestStar) {
        explosions.push(new Explosion(closestStar.screenX, closestStar.screenY, closestStar.baseColor));
